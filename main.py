...
'''Como podemos ver en la bbdd de vuelos sí que hay filas 
...
'''Ahora vamos a hacer un estudio general del tipo de datos
 de cada archivo'''
print(vuelos.info())
'''En el caso de vuelos vemos como la mayoría de los 
datos son int y hay un float. No hay nada raro aquí'''

print(clientes.info())
'''En el caso de clientes podemos ver como la columna postal 
...
'''Ahora vamos a comprobar cómo son los valores de las columnas del 
archivo de vuelos'''
...
#%%
#Haremos el mismo proceso para clientes
...
'''Por lo que podemos observar, el archivo clientes contiene 
los datos de Viajeros de una compañía de viajes canadiense, 
mientras que el archivo vuelos muestra los viajes que ha 
...
'''A continuación procedo a gestionar los nulos que nos hemos 
traído desde la bbdd de clientes
...
sns.boxplot(x = 'salary', data = datos, palette='cool_r')
plt.xlabel('Salario')
plt.title('Boxplot del Salario', fontsize = 15)
...
'''Como podemos observar, a medida que aumenta el nivel educativo
aumenta igualmente el salario medio. Además, el grupo de college
ha desaparecido. Eso puede ser porque probablemente sean clientes
que aún están en edad escolar y no tienen trabajo.'''
...
'''Hay una mayor cantidad de Star, aeguido de Nova y por último
Aurora'''
...
'''Hay una mayor cantidad de clientes casados, seguidos de solteros
y por último, divorciados. En el caso de solteros y divorciados 
hay una muy pequeña diferencia entre hombres y mujeres'''
...
#Primero creo una columna con los grupos
datos['group']=datos['education'].apply(sp.categorizar_grupos)
'''Ahora creo un df con el número total de vuelos por persona y el grupo
al que pertenecen cada  uno'''
...
#Ahora uno las tablas para crear una sola
vuelos_por_persona=vuelos_por_persona.merge(grupo_por_persona, how='inner', on='loyalty_number')

# %%

'''Con .describe podemos ver los principales estadísticos de la 
columna flights_booked'''
vuelos_por_persona['flights_booked'].describe().T

#%%

'''Ahora voy a comenzar el A/B testing empezando por la definición 
de las hipótesis.
· Hipotesis nula[HO]-->No hay cambios significativos entre los dos 
grupos
· Hipotesis alternativa [H1]-->Hay cambios significativos entre 
los dos grupos.
Compruebo si sigue una distribución normal. Uso el test de 
kolmogorov, ya que la muestra era de un tamaño mayor de 50'''
sp.normalidad(vuelos_por_persona,'flights_booked')
sns.histplot(x = 'flights_booked', hue='group',data = vuelos_por_persona,palette='cool_r')
plt.xlabel('Vuelos Reservados')
plt.ylabel('Clientes')
plt.title('Proporción Clientes/Vuelos Reservados', fontsize = 15)

# %%

'''Aunque se ha comprobado que NO siguen una distribución normal 
voy a comprobar la homogeneidad de varianzas utilizando el test de 
Levenne, ya que el de Bartet es más sensible a la falta de 
normalidad '''
sp.homogeneidad(vuelos_por_persona,'group','flights_booked')

#%%

'''Ahora deberíamos elegir el test estadístico idóneo para el A/B
testing. En este caso, al no tener una distribución normal haré un
test de Mann-Whitney'''
sp.test_man_whitney(vuelos_por_persona,'flights_booked','grupo A','grupo B','group')
'''Co el resultado del análisis, al no haber diferencias 
significativas en las medianas de ambos grupos podemos dar
por válida la hipótesis nula, es decir, que no hay una relación 
aparente entre el número de vuelos reservados y el nivel educativo
de los clientes'''
